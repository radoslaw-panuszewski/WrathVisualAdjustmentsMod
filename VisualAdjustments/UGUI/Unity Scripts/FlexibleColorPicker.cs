...
public class FlexibleColorPicker : MonoBehaviour
{
    ...
    */
    ...
    {
        public PSettings r = new PSettings();
        public PSettings g = new PSettings();
        public PSettings b = new PSettings();
        public PSettings h = new PSettings();
        public PSettings s = new PSettings();
        public PSettings v = new PSettings();
        public PSettings a = new PSettings();
    ...
    */
    ...
    private void OnEnable()
    {
        try
        {
            foreach (var text in this.gameObject.GetComponentsInChildren<TextMeshProUGUI>())
            {
                text.color = Color.white;
            }
            var fcptransform = transform;//.Find("Menus/InventoryViewWindow/EEInfo/Toggle/FlexibleColorPicker");
            var FCPComponent = fcptransform.GetComponent<FlexibleColorPicker>();
            foreach (var asd in fcptransform.GetComponents(typeof(Component)))
            {
                if (asd.ToString().Contains("FlexibleColorPicker"))
                {
                    //  FCPComponent = (FlexibleColorPicker)asd;
                }
            }

            var mainpicker = fcptransform.Find("MainPicker");

            var fcpsprite = mainpicker.GetComponent<FCP_SpriteMeshEditor>().sprite;

            var rpicker = fcptransform.Find("RPicker");
            var gpicker = fcptransform.Find("GPicker");
            var bpicker = fcptransform.Find("BPicker");
            var hpicker = fcptransform.Find("HPicker");
            var spicker = fcptransform.Find("SPicker");
            var vpicker = fcptransform.Find("VPicker");
            var apicker = fcptransform.Find("APickerBackground");
            var Preview1 = fcptransform.Find("ColorPreview/OpaquePreview");
            var Preview2 = fcptransform.Find("ColorPreview/PreviewAlphaBackground");
            //VisualAdjustments.Main.logger.Log(gpicker.GetComponent<Image>().material.ToString());
            var pickerlist = new List<FlexibleColorPicker.Picker>();
            {
                var picker = new FlexibleColorPicker.Picker();
                var cmp = mainpicker.GetComponent<Image>();
                picker.image = cmp;
                picker.dynamicSprite = fcpsprite;
                picker.dynamicMaterial = cmp.material;
                pickerlist.Add(picker);
            }
            {
                var picker = new FlexibleColorPicker.Picker();
                picker.image = rpicker.GetComponent<Image>();
                picker.dynamicMaterial = picker.image.material;
                pickerlist.Add(picker);
            }
            {
                var picker = new FlexibleColorPicker.Picker();
                picker.image = gpicker.GetComponent<Image>();
                picker.dynamicMaterial = picker.image.material;
                picker.dynamicSprite = fcpsprite;
                pickerlist.Add(picker);
            }
            {
                var picker = new FlexibleColorPicker.Picker();
                picker.image = bpicker.GetComponent<Image>();
                picker.dynamicMaterial = picker.image.material;
                picker.dynamicSprite = fcpsprite;
                pickerlist.Add(picker);
            }
            {
                var picker = new FlexibleColorPicker.Picker();
                picker.dynamicSprite = fcpsprite;
                picker.image = hpicker.GetComponent<Image>(); picker.dynamicMaterial = picker.image.material;
                picker.dynamicSprite = fcpsprite;
                pickerlist.Add(picker);
            }
            {
                var picker = new FlexibleColorPicker.Picker();
                picker.image = spicker.GetComponent<Image>(); picker.dynamicMaterial = picker.image.material;
                picker.dynamicSprite = fcpsprite;
                pickerlist.Add(picker);
            }
            {
                var picker = new FlexibleColorPicker.Picker();
                picker.image = vpicker.GetComponent<Image>(); picker.dynamicMaterial = picker.image.material;
                picker.dynamicSprite = fcpsprite;
                pickerlist.Add(picker);
            }
            {
                var picker = new FlexibleColorPicker.Picker();
                picker.image = apicker.GetComponent<Image>(); picker.dynamicMaterial = picker.image.material;
                picker.dynamicSprite = fcpsprite;
                pickerlist.Add(picker);
            }
            {
                var picker = new FlexibleColorPicker.Picker();
                picker.image = Preview1.GetComponent<Image>(); picker.dynamicMaterial = picker.image.material;
                picker.dynamicSprite = fcpsprite;
                pickerlist.Add(picker);
            }
            {
                var picker = new FlexibleColorPicker.Picker();
                picker.image = Preview2.GetComponent<Image>(); picker.dynamicMaterial = picker.image.material;
                picker.dynamicSprite = fcpsprite;
                pickerlist.Add(picker);
            }
            //    this.advancedSettings.h.overrideStatic = true;
            this.pickers = pickerlist.ToArray();
        }
        catch (Exception e)
        {
