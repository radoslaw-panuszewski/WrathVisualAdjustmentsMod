...
namespace ModKit {
    public static partial class UI {
        ...
        /*public static void ColorSlider(string RGB,int rgb, ref float col,float h,float s,float v)
        {
            float floatval = col;
            ...
            if(floatval != col)
            {
                ...
                ColorPicker.HexCol = ColorUtility.ToHtmlStringRGB(ColorPicker.col);
                this.changeActiveColor(ColorPicker.col,h,s,v);
            }
            GUILayout.EndHorizontal();
        }*/
        public static float FloatTextFieldClamp(float display,ref float value,float max,float min, String name = null, params GUILayoutOption[] options)
        {
            String text = $"{((Mathf.Round(display * 255)))}";
            UI.TextField(ref text, name, options);
            var val = value;
            if (float.TryParse(text, out val))
            {
                Mathf.Clamp(val,0,255);
                value = val / 255;
            }
